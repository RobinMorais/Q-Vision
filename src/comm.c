...
static uint8_t my_addr = 0x30;
static bool addres_ok = false;
...
bool ready_to_listen = true;
static uint8_t lengt_old_message = 0;
...
static inline uint8_t checksum(const uint8_t *b, uint8_t len_wo_chk) {
    uint8_t s = 0;
    for (uint8_t i = 0; i < len_wo_chk; i++){
        s = s ^ b[i];
    } 
    return (uint8_t)(~s);
}

// ---------- byte parser for incoming messages ----------
static void parse_byte(uint8_t byte) {
    if(!addres_ok){
        if (byte == my_addr) {
            ...
            // Ignore everything until we see our address
            return;
        }
    } else {
        if (count < sizeof(buf)){
...
int comm_send(uint8_t a, uint8_t c, const uint8_t *d, uint8_t l) {
    ...
    if (rc != E_NO_ERROR) {
        // If MSDK does not allow concurrent RX+TX requests,
